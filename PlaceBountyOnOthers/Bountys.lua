...
tes3mp.SendMessage(pid,color.Orange.."[Bountysystem]"..color.Default.." Those are the Bountys set:\n\n",false)
...
for i, Pl in pairs(Players) do  -- print all online players bounty
	if UserConfig.GetValue(Pl.pid, "Bounty") ~= "-1" and UserConfig.GetValue(Pl.pid, "Bounty") ~= -1 then
		...
		tes3mp.SendMessage(pid,Pl.name.." has a bounty of "..BountyGold.. " Gold\n",false)
	end
end
...
if GoldAmount >= gold and gold > 0 then -- if he got more gold than he wants to place
	...
	for i, inv in pairs(Players[pid].data.inventory) do --- remove gold
		if 	inv.refId == "gold_001" then
				...
				--tes3mp.SendMessage(pid,tostring(gold).." Gold.\n",false)
	...
	if UserConfig.GetValue(targetpid, "Bounty") ~= "-1" and UserConfig.GetValue(targetpid, "Bounty") ~= -1 then  -- increase bounty
		...
		tes3mp.SendMessage(targetpid,color.Orange.."[Bountysystem] "..color.Default..Players[targetpid].name.." got a bounty on Top. He now has "..tostring(gold).." Bounty! \n",true)
	else													--- set new bounty
		UserConfig.SetValue(targetpid, "Bounty", tostring(gold))
		tes3mp.SendMessage(targetpid,color.Orange.."[Bountysystem] "..color.Default..Players[targetpid].name.." now got a Bounty. He now has "..tostring(gold).." Bounty! \n",true)
	end
...
if BountyHunterPid ~= -1 then 
	...
	if UserConfig.GetValue(pid, "Bounty") ~= "-1" and UserConfig.GetValue(pid, "Bounty") ~= -1  then  -- check if player got bounty
		...
		tes3mp.SendMessage(BountyHunterPid,color.Orange.."[Bountysystem] "..color.Default..Players[BountyHunterPid].name.." has claimed the bounty for killing "..Players[pid].name.." !! \n",true)
			...
			for i, inv in pairs(Players[BountyHunterPid].data.inventory) do ---add gold if he got gold
				if 	inv.refId == "gold_001" then
					...
					tes3mp.SendMessage(BountyHunterPid,WinBounty.." Gold added.\n",false)
						HeGotGold = true
	...
	tes3mp.SendMessage(pid,color.Orange.."[Bountysystem]"..color.Default.." Processed BountyStuff. You no longer have a Bounty.\n",false)
	tes3mp.SendMessage(BountyHunterPid,color.Orange.."[Bountysystem]"..color.Default.." Processed BountyStuff. You recieved the reward for killing.\n",false)
